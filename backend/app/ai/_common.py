...
def build_advice_prompt(summary: dict) -> str:
    ...
    return (
        ...
        "'Рекомендация:'. "
        "Категория «Переводы» — это перемещения собственных средств, а не "
        "потребительские расходы: игнорируй её при анализе, не выделяй как "
        "статью расходов и не давай по ней рекомендаций. Сумма переводов при "
        "этом включена в общую сумму и в долю остальных категорий — не "
        "исключай её из данных сводки и не меняй процентное соотношение "
        "категорий. "
        "Верни строго JSON: "
        '{"advice": "текст совета"}\n\n'
